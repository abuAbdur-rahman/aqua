...
Repo slice: `daemon/`. A single Rust binary running **inside WSL Ubuntu**, bound to `127.0.0.1`. It's the only thing that touches the real filesystem, process table, and shell — Aqua's Tauri app has no direct OS access at all, it only calls this API.
...
Single binary, `axum` router, bound to `127.0.0.1:8080`. Two consumers, both external to this repo slice:
...
| Phase | Deliverable |
|---|---|
| 0 — Scaffold | `cargo new daemon`; `axum` router with `GET /api/health` and a WS echo route; confirm reachable from Windows via `http://localhost:8080` |
...
`cargo new daemon` inside WSL. Stand up `GET /api/health` and a `/ws/echo` route. Confirm it's reachable from Windows at `http://localhost:8080/api/health` before writing any real fs/pty/sysmon logic — that round-trip is the foundation everything else builds on.
